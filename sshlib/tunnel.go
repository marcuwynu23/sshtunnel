...
func startTunnel(client *ssh.Client, tunnel Tunnel) error {
	...
	log.Printf("Tunneling from remote port %d to local port %d", tunnel.RemotePort, tunnel.LocalPort)
	...
	for {
		...
		go handleTunnel(conn, tunnel.LocalPort)
	}
}

// Function to handle individual tunnel connections
func handleTunnel(conn net.Conn, localPort int) {
	defer conn.Close()
